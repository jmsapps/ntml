when isMainModule and defined(js):
  ...
  proc App(): Node =
    ...
    let shapeItems = signal(@[
      ShapeItem(id: 1, label: "Alpha", expanded: false),
      ShapeItem(id: 2, label: "Beta", expanded: false),
      ShapeItem(id: 3, label: "Gamma", expanded: true)
    ])
    ...
    proc toggleShape(id: int) =
      var cur = shapeItems.get()
      for i in 0 ..< cur.len:
        if cur[i].id == id:
          cur[i].expanded = not cur[i].expanded
          break
      shapeItems.set(cur)
    ...
    AppShell:
      DemoPanel:
        ...
        PanelSection:
          PanelTitle: "Shape change fallback"
          PanelNote:
            "Toggling a row reveals extra controls. Since the keyed fragment gains nodes, the reconciler remounts just that entry."
          KeyedList:
            for shape in shapeItems:
              KeyedItem(key=shape.id):
                ItemHeader:
                  ItemBadge: "#" & $(shape.id)
                  span: shape.label
                ItemButtons:
                  ItemButton(`type`="button", onClick = proc (e: Event) = toggleShape(shape.id)):
                    if shape.expanded:
                      "Hide Detail"
                    else:
                      "Show Detail"
